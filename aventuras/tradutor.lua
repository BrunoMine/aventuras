...
-- Tradução intllib
aventuras.intllib = {}
aventuras.intllib.S, aventuras.intllib.NS = dofile(modpath.."/lib/intllib.lua")

-- Configura tradutor opicional
aventuras.S = aventuras.intllib.S
aventuras.NS = aventuras.intllib.NS
...
-- Gera arquivos de tradução aventuras.*.tr
do
	...
	for _,file in ipairs(list) do
		...
		if string.match(file, "~") == nil then
			...
			if string.match(file, ".pot") == nil and string.match(file, ".po") then
				...
				-- Novo arquivo
				local new_file = "### Arquivo gerado por aventuras apartir de "..file.."\n# textdomain: aventuras\n"
				for en,lang in pairs(en_to_lang) do
					new_file = new_file .. en .. "=" .. lang .. "\n"
				end
				-- Escrever arquivo
				local saida = io.open(modpath.."/locale/aventuras."..lang_code..".tr", "w")
...
if minetest.get_translator ~= nil then
	s = minetest.get_translator("aventuras")
else
	s = aventuras.intllib.S
end

aventuras.s = function(...)
	local args = { ... }
	if pt_to_en[args[1]] ~= nil then
		return s(pt_to_en[args[1]], unpack(args, 2))
	end
	minetest.log("error", "[aventuras] String "..dump(args[1]).." nao catalogada")
	return s(...)
end
	...
	if type(args[1]) == "table" then
		...
		for n,a in ipairs(args[1]) do
			if n ~= 1 then -- Não traduz o primeiro
				table.insert(r, aventuras.S(a))
			else
				table.insert(r, a)
			end
		end
		
		return aventuras.s(unpack(r))
...
aventuras.Sfake = function(s) return s end
