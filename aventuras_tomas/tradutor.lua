...
-- Tradução intllib
aventuras_tomas.intllib = {}
aventuras_tomas.intllib.S, aventuras_tomas.intllib.NS = dofile(modpath.."/lib/intllib.lua")

-- Configura tradutor opicional
aventuras_tomas.S = aventuras_tomas.intllib.S
aventuras_tomas.NS = aventuras_tomas.intllib.NS
...
-- Gera arquivos de tradução aventuras_tomas.*.tr
do
	...
	for _,file in ipairs(list) do
		...
		if string.match(file, "~") == nil then
			...
			if string.match(file, ".pot") == nil and string.match(file, ".po") then
				...
				-- Novo arquivo
				local new_file = "### Arquivo gerado por aventuras_tomas apartir de "..file.."\n# textdomain: aventuras_tomas\n"
				for en,lang in pairs(en_to_lang) do
					new_file = new_file .. en .. "=" .. lang .. "\n"
				end
				-- Escrever arquivo
				local saida = io.open(modpath.."/locale/aventuras_tomas."..lang_code..".tr", "w")
...
if minetest.get_translator ~= nil then
	s = minetest.get_translator("aventuras_tomas")
else
	s = aventuras_tomas.intllib.S
end

aventuras_tomas.s = function(...)
	local args = { ... }
	if pt_to_en[args[1]] ~= nil then
		return s(pt_to_en[args[1]], unpack(args, 2))
	end
	minetest.log("error", "[aventuras_tomas] String "..dump(args[1]).." nao catalogada")
	return s(...)
end
	...
	if type(args[1]) == "table" then
		...
		for n,a in ipairs(args[1]) do
			if n ~= 1 then -- Não traduz o primeiro
				table.insert(r, aventuras_tomas.S(a))
			else
				table.insert(r, a)
			end
		end
		
		return aventuras_tomas.s(unpack(r))
		
	elseif type(args[1]) == "string" then
		-- Não traduz caso faltem argumentos (devido strings ilustrativas)
		return aventuras_tomas.s(...)
...
aventuras_tomas.Sfake = function(s) return s end
