--[[
	Mod aventuras_bob para Minetest
...
-- Tradução intllib
aventuras_bob.intllib = {}
aventuras_bob.intllib.S, aventuras_bob.intllib.NS = dofile(modpath.."/lib/intllib.lua")

-- Configura tradutor opicional
aventuras_bob.S = aventuras_bob.intllib.S
aventuras_bob.NS = aventuras_bob.intllib.NS
...
-- Gera arquivos de tradução aventuras_bob.*.tr
do
	...
	for _,file in ipairs(list) do
		...
		if string.match(file, "~") == nil then
			...
			if string.match(file, ".pot") == nil and string.match(file, ".po") then
				...
				-- Novo arquivo
				local new_file = "### Arquivo gerado por aventuras_bob apartir de "..file.."\n# textdomain: aventuras_bob\n"
				for en,lang in pairs(en_to_lang) do
					new_file = new_file .. en .. "=" .. lang .. "\n"
				end
				-- Escrever arquivo
				local saida = io.open(modpath.."/locale/aventuras_bob."..lang_code..".tr", "w")
...
if minetest.get_translator ~= nil then
	s = minetest.get_translator("aventuras_bob")
else
	s = aventuras_bob.intllib.S
end

aventuras_bob.s = function(...)
	local args = { ... }
	if pt_to_en[args[1]] ~= nil then
		return s(pt_to_en[args[1]], unpack(args, 2))
	end
	minetest.log("error", "[aventuras_bob] String "..dump(args[1]).." nao catalogada")
	return s(...)
end
	...
	if type(args[1]) == "table" then
		...
		for n,a in ipairs(args[1]) do
			if n ~= 1 then -- Não traduz o primeiro
				table.insert(r, aventuras_bob.S(a))
			else
				table.insert(r, a)
			end
		end
		
		return aventuras_bob.s(unpack(r))
		
	elseif type(args[1]) == "string" then
		-- Não traduz caso faltem argumentos (devido strings ilustrativas)
		return aventuras_bob.s(...)
...
aventuras_bob.Sfake = function(s) return s end
